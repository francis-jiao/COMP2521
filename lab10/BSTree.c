...
void BSTreeLevelOrder(BSTree t)
{
    ...
    while (!QueueIsEmpty(newQ)) {
        BSTLink temp = QueueLeave(newQ);
        printf("%d ", temp -> value);
        if (temp -> left != NULL) {
            QueueJoin(newQ, temp -> left);
        }
        if (temp -> right != NULL) {
            QueueJoin(newQ, temp -> right);
        }
    }
    return;
}
...
int BSTreeNumLeaves(BSTree t)
{
	...
	if (t -> left != NULL) l = BSTreeNumLeaves(t -> left);

	if (t -> right != NULL) r = BSTreeNumLeaves(t -> right);
	count = l + r;
	return count;
}
